...
# run the SQL commands from the mysql_secure_installation script except for setting the root password (so that we don't embed) the root password in our provisioning script
mysql -u root <<-EOF
